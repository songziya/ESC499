...
class ReservoirVisualizer(Visualizer):
    '''
    Scott:
    ...
    def render(self,
        ...
        idx = self._compiler.state_fluent_ordering.index('rlevel/1')
        #print(idx)
        start = initial_state[idx][0]
        #print(initial_state[idx])
        #print(start)
        
        g = non_fluents['desiredlvl/1']
        #print(g)
        
        path = states['rlevel/1']
        #print(path)
## select fluent to extract action from:
        deltas = actions['outflow/1'] # v1, v3
        #deltas = interms['outflowBound/1'] # v2
...
##        
        ...
        lime_line = mlines.Line2D([0],[0], color ='w', markerfacecolor='limegreen', marker='X', markersize = 15, label='initial waterlevel')
        crim_line = mlines.Line2D([0],[0], color ='w', markerfacecolor='crimson', marker='X', markersize=15, label='desired waterlevel')
        ...
        for i in range(0,res):
            self._render_start_and_goal_positions(start[i], g[i], horizon)
